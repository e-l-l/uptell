...
def get_log(incident_id: int, log_id: int, supabase=Depends(get_supabase)):
    res = supabase.table("incident_logs").select("*").eq("id", log_id).eq("incident_id", incident_id).execute()
    if not res.data:
...
def update_log(incident_id: int, log_id: int, payload: LogUpdate, supabase=Depends(get_supabase)):
    ...
    res = supabase.table("incident_logs").update(update_data).eq("id", log_id).eq("incident_id", incident_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Log not found")
    return res.data[0]
...
def delete_log(incident_id: int, log_id: int, supabase=Depends(get_supabase)):
    res = supabase.table("incident_logs").delete().eq("id", log_id).eq("incident_id", incident_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Log not found")
